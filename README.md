...
| `PROXY_URL` | 代理 / 反代配置，支持正向代理、万能反代、平台专用反代；平台专用字段包括 `bahamut@`、`tmdb@`、`bilibili@`、`animeko@`。Animeko 的 Bangumi 搜索未配置时会内置优先尝试 `https://api.bangumi.one` 并保留直连兜底；显式 `animeko@` 可覆盖 Bangumi 搜索、Animeko V2 详情以及 openani/myani 弹幕节点 |
